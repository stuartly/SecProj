...
public:
    ...
    PointToAnalysis(ASTManager *astmanager, InterProcedureCFG *interProcedureCFG,
                    unordered_map<string, string> configure)
  ...
  std::set<const PTAVar *> get_alias_in_func(FunctionDecl *FD,
                                             const PTAVar *var);
  ...
  void FindPath(std::vector<ASTFunction *> functions);
  std::vector<std::list<const CFGBlock *>> getFuncPath(ASTFunction*);
...
private:
    ...
    std::unordered_map<ASTFunction*,std::vector<std::list<const CFGBlock *>>>funcPath;
};
