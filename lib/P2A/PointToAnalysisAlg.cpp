...
std::set<const PTAVar *> PointToAnalysis::get_alias_in_func(FunctionDecl *FD,
                                                            const PTAVar *var) {
  if (nullptr == var) {
    std::string red = "\033[31m";
    std::string close = "\033[0m";
  ...
  std::vector<std::list<const CFGBlock *>> paths = getPathInFunc(entry, exit);
  for (auto paths_it = paths.begin(), paths_itend = paths.end();
       paths_it != paths_itend; ++paths_it) {

    FindVariable findVariable(var->get_type()->getName());
