...
void PointToAnalysis::ConstructPartialCFG() {
  ...
  while (f_iter != f_iter_end) {
    ...
    // std::vector<std::list<const CFGBlock *>> path =
...
void PointToAnalysis::FindPath(std::vector<ASTFunction *> functions) {
  //auto functions = manager->getFunctions();
  auto f_iter = functions.begin();
  auto f_iter_end = functions.end();
  // handle all functions
  ...
  while (f_iter != f_iter_end) {
    ASTFunction* F = const_cast<ASTFunction*>(*f_iter);
    ...
    // path generate test
    //std::unique_ptr<CFG> &fd_cfg = manager->getCFG(*f_iter);
    // ASTFunction* Fss = const_cast<ASTFunction*>(*f_iter);
    // cout << "start analyze path in function " + Fss->getFunctionName() << endl;

    std::vector<std::list<const CFGBlock *>> path =
        getPathInFunc(&(fd_cfg->getEntry()), &(fd_cfg->getExit()));
    funcPath.insert(pair<ASTFunction*,std::vector<std::list<const CFGBlock *>>>(F,path));
...
std::vector<std::list<const CFGBlock *>> PointToAnalysis::getFuncPath(ASTFunction* F) {
    if(funcPath.find(F) != funcPath.end()){
      return funcPath[F];
    }
    return vector<std::list<const CFGBlock *>>();
}
...
PointToAnalysis::getPathInFunc(CFGBlock *startBlock, CFGBlock *endBlock) {
  ...
  while (iter != childs.end()) {
    traverseCFGForGetReachablePath(reachablePath, *iter, endBlock, path);
    ++iter;
  }
  return reachablePath;
}
...
void PointToAnalysis::Alias_test2() {
  ...
  std::set<const PTAVar *> res = get_alias_in_func(FD, var);
