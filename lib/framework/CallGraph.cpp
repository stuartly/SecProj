...
CallGraph::CallGraph(
  ...
  for (ASTFunction *F : resource.getFunctions()) {
    ...
    if (nodes.find(fullName) != nodes.end()) {
      ...
      if (node->getParents().size() == 0) {
        ...
        if (ignoreSystemHeader == "true" && node->getChildren().size()) {
  ...
  std::string printDot = config.at("printToDot");
  if (printDot == "true") {
    std::ofstream cg_dot_file("cg.dot");
    if (!cg_dot_file.is_open())
    {
      std::cerr << "can't open time.txt\n";
      return;
    }
    writeDotFile(cg_dot_file);
    cg_dot_file.close();
  }
}
